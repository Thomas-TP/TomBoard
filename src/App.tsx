...
function App() {
  ...
  const handleKeyDown = useCallback(
    (e: KeyboardEvent) => {
      ...
      if (e.ctrlKey && e.key === "3") {
        e.preventDefault();
        useAppStore.getState().setViewMode("compact");
      }
    ...
    const syncAudio = async () => {
      ...
      if (!secDevice || secDevice === "none") {
        try {
          ...
          console.warn("[STARTUP] check_virtual_cable failed:", e);
        }
      }
      ...
      if (secDevice && secDevice !== "none") {
        try {
          ...
          console.error("[STARTUP] set_secondary_device FAILED:", e);
        }
        try {
          await invoke("set_dual_output", { enabled: true });
          console.log("[STARTUP] set_dual_output OK: true");
        } catch (e) {
          console.error("[STARTUP] set_dual_output FAILED:", e);
        }
        ...
        if (micDev) {
          try {
            ...
            console.error("[STARTUP] start_mic_passthrough FAILED:", e);
          }
        }
        // Sync silent mode
      ...
      <ThemeProvider theme={theme}>
        ...
        <Box
          sx={{
            ...
            gap: 3,
            backgroundImage:
              "radial-gradient(ellipse 80% 50% at 50% 30%, rgba(124, 92, 252, 0.12) 0%, transparent 70%)",
          }}
        >
          <Box
            sx={{
  ...
  return (
    <ThemeProvider theme={theme}>
      ...
      <AddSoundDialog open={addDialogOpen} onClose={() => setAddDialogOpen(false)} />

      <SoundLibraryDialog open={libraryOpen} onClose={() => setLibraryOpen(false)} />
      ...
      <ErrorBoundary fallbackTitle={t("voiceChangerError")}>
        <VoiceChangerPanel open={voiceChangerOpen} onClose={() => setVoiceChangerOpen(false)} />
      </ErrorBoundary>
      ...
      <Snackbar open={!!updateAvailable} anchorOrigin={{ vertical: "bottom", horizontal: "right" }}>
        <Alert
          severity="info"
          variant="filled"
          onClose={dismiss}
          action={
            <>
              ...
              <Button color="inherit" size="small" disabled={updating} onClick={applyUpdate}>
