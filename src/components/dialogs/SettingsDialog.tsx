...
export default function SettingsDialog({ open, onClose }: SettingsDialogProps) {
  ...
  const [ieMessage, setIeMessage] = useState<{ type: "success" | "error"; text: string } | null>(
    null,
  );
  const [virtualCables, setVirtualCables] = useState<string[]>([]);
  const [vcInstalling, setVcInstalling] = useState(false);
  const [vcMessage, setVcMessage] = useState<{ type: "success" | "error"; text: string } | null>(
    null,
  );
  ...
  const [piperStatus, setPiperStatus] = useState<
    "idle" | "detecting" | "installing" | "downloading-model" | "done" | "error"
  >("idle");
  ...
  useEffect(() => {
    if (open && data) {
      ...
      invoke<string[]>("list_audio_devices")
        ...
        .catch(() => setAudioDevices(["default"]));
      invoke<string[]>("list_audio_input_devices")
        .then(setMicInputDevices)
        .catch(() => setMicInputDevices(["default"]));
      invoke<string>("get_app_data_dir")
        .then(setAppDataDir)
        .catch(() => {});
      invoke<string[]>("check_virtual_cable")
        .then(setVirtualCables)
        .catch(() => setVirtualCables([]));
  ...
  const handleSave = async () => {
    if (settings) {
      ...
      await invoke("set_master_volume", { volume: finalSettings.masterVolume }).catch(
        console.error,
      );
      await invoke("set_output_device", { deviceName: finalSettings.outputDevice }).catch(
        console.error,
      );
      ...
      if (finalSettings.noiseSuppression !== undefined) {
        await invoke("set_noise_suppression", { enabled: finalSettings.noiseSuppression }).catch(
          console.error,
        );
      }
      ...
      if (finalSettings.dualOutput && finalSettings.micPassthroughDevice) {
        ...
        if (cable && cable !== "none") {
          await invoke("start_mic_passthrough", {
            inputDevice: finalSettings.micPassthroughDevice,
            outputDevice: cable,
          }).catch(console.error);
        }
      }
  ...
  const handleDuplicateProfile = async (id: string) => {
    ...
    if (source) await duplicateProfile(id, `${source.name} (copie)`);
  };
  const startEditingProfile = (id: string) => {
    setProfileMenuAnchor(null);
  ...
  const handleExport = async () => {
    try {
      ...
      setIeMessage({ type: "error", text: `${t("exportError")} ${e}` });
    } finally {
      setIeLoading(false);
    }
  };
  ...
  const handleImport = async () => {
    try {
      ...
      setIeMessage({ type: "error", text: `${t("importError")} ${e}` });
    } finally {
      setIeLoading(false);
    }
  };
  ...
  return (
    ...
    >
      ...
      >
        ...
        >
          <SettingsIcon sx={{ fontSize: 20 }} /> {t("settings")}
        </Typography>
      ...
      <Box sx={{ display: "flex", flex: 1, overflow: "hidden" }}>
        ...
        >
          {(
            ...
            ] as [number, string, React.ReactNode][]
          ).map(([idx, label, icon]) => (
            <ListItemButton
              key={idx}
              selected={tab === idx}
              ...
              sx={{
                ...
                "&.Mui-selected": {
                  ...
                  color: "primary.main",
                  "&:hover": { bgcolor: "rgba(124, 92, 252, 0.16)" },
                },
              }}
            >
              {icon}
              <Typography sx={{ fontSize: "0.82rem", fontWeight: tab === idx ? 600 : 400 }}>
                {label}
              </Typography>
            </ListItemButton>
          ))}
        ...
        >
          {tab === 0 && (
            <>
              ...
              <Box>
                ...
                <Box sx={{ display: "flex", flexWrap: "wrap", gap: 0.75 }}>
                  {SEED_COLORS.map((color) => (
                    <Box
                      key={color}
                      onClick={() => update({ customSeedColor: color })}
                      sx={{
                        width: 28,
                        height: 28,
                        borderRadius: "50%",
                        ...
                        border: 2,
                        borderColor:
                          settings.customSeedColor === color ? "text.primary" : "transparent",
              ...
              <Box>
                ...
                >
                  {t("customSoundsFolderHint")}
                </Typography>
                <TextField
                  value={settings.soundsFolder}
                  onChange={(e) => update({ soundsFolder: e.target.value })}
                  size="small"
                  fullWidth
              ...
              <Box>
                ...
                <FormControlLabel
                  ...
                  label={<Typography variant="body2">{t("minimizeToTray")}</Typography>}
                />
                <FormControlLabel
                  control={
                    <Switch
                      checked={settings.launchMinimized}
                      onChange={(e) => update({ launchMinimized: e.target.checked })}
                      size="small"
                    />
                  }
                  label={<Typography variant="body2">{t("launchMinimized")}</Typography>}
                />
                <FormControlLabel
                  control={
                    <Switch
                  ...
                  label={<Typography variant="body2">{t("discordRpc")}</Typography>}
                />
              </Box>
              <Divider />
              <Box>
                  ...
                  {(["fr", "en"] as const).map((lang) => (
                    <Box
                      ...
                      sx={{
                        ...
                        border: "1px solid",
                        borderColor:
                          (settings.language ?? "fr") === lang ? "primary.main" : "divider",
                        bgcolor:
                          (settings.language ?? "fr") === lang ? "primary.main" : "transparent",
                        color:
                          (settings.language ?? "fr") === lang
                ...
                {settings.piperPath && settings.piperModel ? (
                  ...
                  >
                    <Button
                      ...
                      disabled={
                        piperStatus === "detecting" ||
                        piperStatus === "installing" ||
                        piperStatus === "downloading-model"
                      }
                      startIcon={
                        piperStatus !== "idle" &&
                        piperStatus !== "done" &&
                    ...
                    {piperMessage && (
                      <Alert
                        ...
                        sx={{ borderRadius: 2, py: 0.25, width: "100%" }}
                      >
                        {piperMessage}
                      </Alert>
                    )}
            ...
            <>
              ...
              <Box sx={{ display: "flex", alignItems: "center", gap: 1.5 }}>
                ...
                <Slider
                  ...
                  onChange={(_, v) => update({ masterVolume: v as number })}
                  min={0}
                  max={1}
                  step={0.01}
                  size="small"
              ...
              <FormControl fullWidth size="small">
                ...
                >
                  {audioDevices
                    .filter((dev) => !virtualCables.includes(dev))
                    .map((dev) => (
          ...
          {tab === 2 && (
            <>
              ...
              >
                <MicIcon sx={{ fontSize: 16 }} /> {t("audioIntegration")}
              </Typography>
              ...
              {virtualCables.length > 0 ? (
                <>
                  ...
                  <FormControlLabel
                    control={
                      <Switch
                        checked={settings.dualOutput}
                        onChange={(e) => {
                          const enabled = e.target.checked;
                          update({
                            dualOutput: enabled,
                            secondaryDevice: enabled ? virtualCables[0] : "none",
                          });
                        }}
                        size="small"
                      />
                  ...
                  {settings.dualOutput && (
                    <>
                      <Alert severity="info" sx={{ fontSize: "0.7rem", py: 0.5 }}>
                        {t("soundsBroadcastHint").replace("{cable}", virtualCables[0])}
                        <br />
                      ...
                      {vcMessage && (
                        <Alert
                          severity={vcMessage.type}
                          onClose={() => setVcMessage(null)}
                      ...
                      <Alert severity="info" sx={{ fontSize: "0.7rem", py: 0.5 }}>
                        {t("micRedirectHint").replace("{cable}", virtualCables[0])}
                        <br />
                      ...
                      />
                      <Alert severity="info" sx={{ fontSize: "0.7rem", py: 0.5 }}>
                        <strong>{t("discordTip")}</strong> {t("discordNoiseHint")}
                        <br />
                        {t("discordNoiseHint2")}
                      </Alert>
                    </>
            ...
            <>
              ...
              {profile && (
                <Alert
                  severity="info"
                  icon={false}
                  sx={{ py: 0.5, borderRadius: "10px", fontSize: "0.75rem" }}
                >
              ...
              <Box>
                <Typography
                  ...
                  sx={{ mb: 0.5, display: "block" }}
                >
                  {t("iconLabel")} {renderCategoryIcon(newCatIcon, { sx: { fontSize: 18 } })}
                </Typography>
                    ...
                    <Grid key={opt.id} size={{ xs: "auto" }}>
                      <Box
                        ...
                        sx={{
                          ...
                          borderRadius: 1,
                          cursor: "pointer",
                          bgcolor: newCatIcon === opt.id ? "primary.main" : "transparent",
                          color: newCatIcon === opt.id ? "primary.contrastText" : "inherit",
                          "&:hover": { bgcolor: "action.hover" },
                          fontSize: "1.1rem",
                        }}
                        title={t(opt.i18nKey as any)}
                      >
                ...
                {profiles.map((p) => (
                  ...
                  >
                    <ListItemIcon sx={{ minWidth: 40 }}>
                      <Avatar
                        sx={{
                          ...
                          height: 30,
                          fontSize: "0.8rem",
                          bgcolor: p.id === activeProfileId ? "primary.main" : "action.hover",
                        }}
                      >
                    ...
                    {editingProfileId === p.id ? (
                      ...
                      <ListItemText
                        primary={p.name}
                        secondary={`${p.sounds.length} ${t("sounds")} · ${p.categories.length} ${t("categories").toLowerCase()}`}
                        slotProps={{
                          primary: { sx: { fontWeight: p.id === activeProfileId ? 700 : 500 } },
                    ...
                    <ListItemSecondaryAction>
                      ...
                      <IconButton
                        ...
                        onClick={(e) => {
                          e.stopPropagation();
                          setProfileMenuAnchor({ el: e.currentTarget, profileId: p.id });
                        }}
                      >
          ...
          {tab === 5 && (
            <>
              ...
              {ieMessage && (
                <Alert severity={ieMessage.type} onClose={() => setIeMessage(null)}>
                  {ieMessage.text}
                </Alert>
              )}
          ...
          {tab === 6 && (
            <>
              ...
              {updateStatus === "downloading" && (
                <>
                  <Alert severity="info" sx={{ fontSize: "0.8rem" }}>
                    {t("downloadingUpdate")} v{updateVersion}...
                  </Alert>
                  <LinearProgress />
                </>
              )}
              ...
              <Box sx={{ display: "flex", gap: 2 }}>
                ...
                {updateVersion && (
                  ...
                  >
                    {updateStatus === "downloading"
                      ? t("installing")
                      : `${t("installVersion")} v${updateVersion}`}
                  </Button>
                )}
              </Box>
              <Typography variant="caption" color="text.secondary">
      ...
      <DialogActions
        sx={{ px: 3, py: 2, flexShrink: 0, borderTop: "1px solid", borderColor: "divider" }}
      >
      ...
      <Menu
        anchorEl={profileMenuAnchor?.el}
        open={!!profileMenuAnchor}
        onClose={() => setProfileMenuAnchor(null)}
      >
        <MenuItem
          onClick={() => profileMenuAnchor && startEditingProfile(profileMenuAnchor.profileId)}
        >
          <Edit sx={{ fontSize: 18, mr: 1 }} /> {t("rename")}
        </MenuItem>
        <MenuItem
          onClick={() => profileMenuAnchor && handleDuplicateProfile(profileMenuAnchor.profileId)}
        >
          <ContentCopy sx={{ fontSize: 18, mr: 1 }} /> {t("duplicate")}
        </MenuItem>
        {profiles.length > 1 && (
          <MenuItem
            onClick={() => profileMenuAnchor && handleDeleteProfile(profileMenuAnchor.profileId)}
            sx={{ color: "error.main" }}
          >
