...
export default function SoundLibraryDialog({ open, onClose }: SoundLibraryDialogProps) {
  ...
  const doSearch = useCallback(
    async (searchQuery: string, isDefault = false, src?: LibrarySource) => {
  ...
  const stopPreview = async () => {
    ...
    try {
      await invoke("stop_preview_library");
    } catch {
      /* ignore */
    }
    setPlayingKey(null);
  ...
  return (
    ...
    >
      ...
      >
        ...
        {loading && (
          <Box
            sx={{ display: "flex", flexDirection: "column", gap: 0.75, overflow: "auto", flex: 1 }}
          >
            {[...Array(5)].map((_, i) => (
              <Box
                key={i}
            ...
            {results.map((sound) => {
              ...
              return (
                ...
                >
                  ...
                  {/* Info */}
                  <Box sx={{ flex: 1, minWidth: 0 }} onClick={() => togglePreview(sound)}>
                    <Typography
                      variant="body2"
                      ...
                      {sound.num_downloads > 0 && (
                        <Box sx={{ display: "flex", alignItems: "center", gap: 0.3 }}>
                          ...
                          <Typography
                            ...
                            sx={{ fontSize: "0.65rem" }}
                          >
                            {sound.num_downloads > 1000
                              ? `${(sound.num_downloads / 1000).toFixed(1)}k`
                              : sound.num_downloads}
                          </Typography>
                        </Box>
                      )}
                  ...
                  {isAdded ? (
                    ...
                    <Button
                      ...
                      disabled={isAdding}
                      startIcon={
                        isAdding ? <CircularProgress size={12} /> : <Add sx={{ fontSize: 14 }} />
                      }
                      sx={{
                        borderRadius: "8px",
                        textTransform: "none",
