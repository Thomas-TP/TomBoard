...
export default function AddSoundDialog({ open: isOpen, onClose }: AddSoundDialogProps) {
  ...
  useEffect(() => {
      ...
      invoke<TtsVoice[]>("list_tts_voices")
        .then((voices) => {
          ...
          const deduped = voices.filter((v) =>
            seen.has(v.name) ? false : (seen.add(v.name), true),
          );
    ...
    if (isOpen && data?.settings.piperPath) {
      invoke<boolean>("check_piper", { piperPath: data.settings.piperPath })
        .then((ok) => {
  ...
  const availableLangs: { code: string; label: string }[] = [
    ...new Set(ttsVoices.map((v) => langCodeOf(v.culture))),
  ]
    .sort()
    .map((code) => ({
      code,
  ...
  const filteredVoices = ttsVoices.filter((v) => {
    ...
    if (ttsGenderFilter !== "All" && v.gender !== ttsGenderFilter && v.gender !== "Unknown")
      return false;
    return true;
  });
  ...
  const startRecording = async () => {
    try {
      ...
      if (!recName) {
        setRecName(
          `${t("recording")} ${new Date().toLocaleTimeString(undefined, { hour: "2-digit", minute: "2-digit" })}`,
        );
      }
    } catch (e) {
  ...
  const detectLanguage = (text: string): string => {
    ...
    const wordSets: [Set<string>, string][] = [
      [frWords, "fr"],
      [enWords, "en"],
      [esWords, "es"],
      [deWords, "de"],
    ...
    try {
      if (ttsEngine === "piper") {
        ...
        await invoke("synthesize_speech", {
          ...
          voiceName: ttsVoice,
          rate: ttsRate,
          engine: selectedVoice?.engine || "SAPI",
        });
      }
      await loadData();
      onClose();
          ...
          <Box sx={{ display: "flex", flexDirection: "column", gap: 2 }}>
            ...
            {ttsError && (
              <Alert
                ...
                sx={{ fontSize: "0.75rem", py: 0.5, borderRadius: "10px" }}
              >
                {ttsError}
              </Alert>
            )}
          ...
          <Box sx={{ display: "flex", flexDirection: "column", gap: 2 }}>
            ...
            {ttsDetectedLang && ttsEngine === "windows" && (
              <Box sx={{ display: "flex", alignItems: "center", gap: 0.75 }}>
                ...
                <Chip
                  label={LANG_LABELS[ttsDetectedLang] || ttsDetectedLang.toUpperCase()}
                  size="small"
                  color="info"
                  sx={{ height: 22, fontSize: "0.65rem", borderRadius: "6px" }}
                />
              </Box>
            )}
            ...
            {ttsEngine === "windows" && (
              <Box sx={{ display: "flex", gap: 1, alignItems: "center", flexWrap: "wrap" }}>
                ...
                >
                  <ToggleButton value="All" sx={{ textTransform: "none", fontSize: "0.7rem" }}>
                    Tous
                  </ToggleButton>
                  <ToggleButton value="Female">
                    <Female sx={{ fontSize: 14 }} />
                  </ToggleButton>
            ...
            {ttsEngine === "windows" && (
              <>
                <FormControl size="small" fullWidth>
                  ...
                  <Select
                    value={
                      filteredVoices.find((v) => v.name === ttsVoice)
                        ? ttsVoice
                ...
                <Box>
                  <Typography variant="body2" sx={{ fontSize: "0.78rem", mb: -0.5 }}>
                    Vitesse : {ttsRate > 0 ? `+${ttsRate}` : ttsRate}
                  </Typography>
                  <Slider
                    value={ttsRate}
                    onChange={(_, v) => setTtsRate(v as number)}
                    min={-5}
                    max={5}
                    step={1}
                    marks
            ...
            <Button
              ...
              disabled={
                !ttsText.trim() ||
                (ttsEngine === "windows" && !ttsVoice) ||
                (ttsEngine === "piper" && !piperAvailable) ||
                ttsLoading
              }
              fullWidth
              startIcon={
                ttsLoading ? <CircularProgress size={16} /> : <GraphicEq sx={{ fontSize: 18 }} />
              }
              sx={{ borderRadius: "10px", py: 1, textTransform: "none", fontWeight: 600 }}
            >
